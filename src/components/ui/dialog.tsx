...
>(({ className, ...props }, ref) => (
    <DialogPrimitive.Overlay
        ref={ref}
        className={cn(
            // v3: lighter overlay (foreground/45)
...
>(({ className, children, onPointerDownOutside, onInteractOutside, onCloseAutoFocus, onOpenAutoFocus, ...props }, ref) => (
    <DialogPortal>
        ...
        <DialogPrimitive.Content
            ref={ref}
            className={cn(
                // v3: pop shadow on dialogs (sm:rounded-xl = 18px)
                "fixed left-[50%] top-[50%] z-50 grid max-h-[85vh] w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 overflow-y-auto border border-border bg-card p-6 text-card-foreground shadow-pop duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-xl",
                className
            )}
            onCloseAutoFocus={(e) => {
