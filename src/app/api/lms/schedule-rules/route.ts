...
import { createScheduleRuleForAcademy } from '@/lib/lms/mutations';
import type { CreateScheduleRuleInput } from '@/features/lms/types';

export async function POST(request: Request) {
    try {
        assertSameOrigin(request);
        const body = await request.json() as { academyId?: string; input?: CreateScheduleRuleInput };
        ...
        await createScheduleRuleForAcademy(body.academyId, body.input);
