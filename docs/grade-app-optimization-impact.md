...
작성일: 2026-07-10
상태: 2026-07-10 Supabase Optimization v2 운영 적용 완료. 영향 분석 및 후속 구현 계약이며, 이번 작업에서 `C:\codes\nextum\grade-app`은 수정하지 않았다.

## 결론

Supabase Optimization v2는 Grade App 변경이나 재배포 없이 운영 DB에 적용했다. 기존 Grade App compatibility table, legacy content, 공개 함수 signature, v1 Realtime 이벤트를 유지한다.
...
운영 DB 비밀번호를 재설정한 경우 Grade App 또는 배포 환경에 direct PostgreSQL connection string이 있는지 확인해 해당 secret만 새 비밀번호로 갱신한다. Supabase URL과 publishable/secret key를 사용하는 Data API 경로는 DB 비밀번호 재설정 대상이 아니다.
